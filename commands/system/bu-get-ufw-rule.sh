...
then
    bu_autohelp \
        --description "Show UFW firewall status and rules (PowerShell Get-NetFirewallRule analog). Wraps ufw status; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --ufw 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_ufw_rule_main "$@"
