...
__bu_bu_stop_service_complete_units()
{
    BU_RET=()
    local u
    while IFS= read -r u
...
function __bu_bu_stop_service_main()
{
...
do
    ...
    --unit)# UNIT
        ...
        bu_parse_positional $# --ret __bu_bu_stop_service_complete_units ret-- --hint "Unit name"
        units+=("${!shift_by}")
        ;;
    -h|--help)# _FLAG
        # Print help
    ...
    *)
        ...
        then
            ...
            autocompletion=(--ret __bu_bu_stop_service_complete_units ret-- --hint "Unit name")
        fi
        units+=("$1")
        ;;
    esac
...
then
    bu_autohelp \
        --description "
Stop systemd service units (PowerShell Stop-Service analog, structured systemctl stop).
Emits one record per unit: unit, action, stopped (boolean); failures carry
...
if ((${#units[@]} == 0))
then
    error_msg="Missing required unit name (e.g. bu stop-service sshd)"
    bu_autohelp
...
local unit err
{
    ...
    do
        if "$is_what_if"
        then
            bu_log_info "What if: systemctl ${systemctl_args[*]} stop $unit"
            continue
        fi
        if err=$(systemctl "${systemctl_args[@]}" stop -- "$unit" 2>&1)
        then
            bu_out_record unit="$unit" action=stop stopped:=true
        else
            bu_out_record unit="$unit" action=stop stopped:=false error="$err"
            rc=1
        fi
    done
} > "$records_file"
