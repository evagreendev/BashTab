...
function __bu_bu_restart_service_main()
{
...
do
    ...
    --unit)# UNIT
        ...
        bu_parse_positional $# --ret __bu_bu_restart_service_complete_units ret-- --hint "Unit name"
        units+=("${!shift_by}")
        ;;
    -h|--help)# _FLAG
        # Print help
    ...
    *)
        ...
        then
            ...
            autocompletion=(--ret __bu_bu_restart_service_complete_units ret-- --hint "Unit name")
        fi
        units+=("$1")
        ;;
    esac
...
then
    bu_autohelp \
        --description "
Restart systemd service units (PowerShell Restart-Service analog, structured systemctl restart).
Emits one record per unit: unit, action, restarted (boolean); failures carry
...
if ((${#units[@]} == 0))
then
    error_msg="Missing required unit name (e.g. bu restart-service sshd)"
    bu_autohelp
...
local unit err
{
    ...
    do
        if "$is_what_if"
        then
            bu_log_info "What if: systemctl ${systemctl_args[*]} restart $unit"
            continue
        fi
        if err=$(systemctl "${systemctl_args[@]}" restart -- "$unit" 2>&1)
        then
            bu_out_record unit="$unit" action=restart restarted:=true
        else
            bu_out_record unit="$unit" action=restart restarted:=false error="$err"
            rc=1
        fi
    done
} > "$records_file"
