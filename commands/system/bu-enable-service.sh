...
function __bu_bu_enable_service_main()
{
...
do
    ...
    --user)# _FLAG
        ...
        ;;
    --now)# _FLAG
        # Also start the unit immediately (systemctl enable --now)
        is_now=true
        ;;
    --what-if)# _FLAG
    ...
    --unit)# UNIT
        ...
        bu_parse_positional $# --ret __bu_bu_enable_service_complete_units ret-- --hint "Unit name"
        units+=("${!shift_by}")
        ;;
    -h|--help)# _FLAG
        # Print help
    ...
    *)
        ...
        then
            ...
            autocompletion=(--ret __bu_bu_enable_service_complete_units ret-- --hint "Unit name")
        fi
        units+=("$1")
        ;;
    esac
...
then
    bu_autohelp \
        --description "
Enable systemd service units (PowerShell Enable-Service analog, structured systemctl enable).
Emits one record per unit: unit, action, enabled (boolean); failures carry
...
Use --now to also start the unit after enabling.
" \
        --example "One unit" "sshd" \
        --example "Enable and start" "nginx --now" \
...
if ((${#units[@]} == 0))
then
    error_msg="Missing required unit name (e.g. bu enable-service sshd)"
    bu_autohelp
...
local unit err
{
    ...
    do
        if "$is_what_if"
        then
            bu_log_info "What if: systemctl ${systemctl_args[*]} enable $unit"
            continue
        fi
        if err=$(systemctl "${systemctl_args[@]}" enable -- "$unit" 2>&1)
        then
            bu_out_record unit="$unit" action=enable enabled:=true
        else
            bu_out_record unit="$unit" action=enable enabled:=false error="$err"
            rc=1
        fi
    done
} > "$records_file"
