#!/usr/bin/env bash
# Synopsis: Show kernel parameters
function __bu_bu_get_sysctl_main()
{
...
then
    bu_autohelp \
        --description "Show kernel parameters (sysctl, PowerShell Get-ItemProperty analog). Wraps sysctl; extra arguments replace the default arguments." \
        --example "Default" "" \
        --example "With arguments" "kernel.hostname"
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --sysctl 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_sysctl_main "$@"
