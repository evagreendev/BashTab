...
then
    bu_autohelp \
        --description "
Parse an ASCII table on stdin (e.g. mysql or psql query output) into records, one per row.
Wraps jc --asciitable. For the reverse direction see bu convert-to-jsonl.
" \
        --example "Default" ""
    return 0
fi
...
jc --asciitable 2>/dev/null | jq -c 'if type == "array" then .[] else . end' | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_convert_from_asciitable_main "$@"
