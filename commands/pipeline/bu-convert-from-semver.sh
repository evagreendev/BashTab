...
then
    bu_autohelp \
        --description "
Parse a semantic version string from stdin into a record (major, minor, patch, prerelease, build).
Wraps jc --semver. For the reverse direction see bu convert-to-jsonl.
" \
        --example "Default" ""
    return 0
fi
...
jc --semver 2>/dev/null | jq -c 'if type == "array" then .[] else . end' | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_convert_from_semver_main "$@"
