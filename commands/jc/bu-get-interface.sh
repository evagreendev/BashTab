...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v ifconfig &>/dev/null || { echo "ifconfig is required" >&2; exit 1; }
    exit 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --ifconfig 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_interface_main "$@"
