...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v wc &>/dev/null || { echo "wc is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Count lines, words, and characters (jc wc parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --wc 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_count_main "$@"
