...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v traceroute &>/dev/null || { echo "traceroute is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Trace the network route to a host (traceroute). Wraps traceroute; extra arguments replace the default arguments." \
        --example "Default" "" \
        --example "With arguments" "example.com"
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --traceroute 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_trace_route_main "$@"
