...
then
    bu_autohelp \
        --description "Show the kernel IP routing table (PowerShell Get-NetRoute). Wraps ip route; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --ip-route 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_route_main "$@"
