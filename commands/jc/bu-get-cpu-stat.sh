...
then
    bu_autohelp \
        --description "Show CPU and IO statistics (jc iostat parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --iostat 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_cpu_stat_main "$@"
