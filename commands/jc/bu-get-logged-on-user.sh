...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v who &>/dev/null || { echo "who is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "List users currently logged in (who). Wraps who; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --who 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_logged_on_user_main "$@"
