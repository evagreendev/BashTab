...
then
    bu_autohelp \
        --description "List filesystem mount table entries (/etc/fstab). Wraps cat /etc/fstab; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --fstab 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_fstab_entry_main "$@"
