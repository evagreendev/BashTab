#!/usr/bin/env bash
# Synopsis: List entries from the hosts file
function __bu_bu_get_host_file_entry_main()
{
...
then
    bu_autohelp \
        --description "List static host entries (/etc/hosts). Wraps cat /etc/hosts; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --hosts 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_host_file_entry_main "$@"
