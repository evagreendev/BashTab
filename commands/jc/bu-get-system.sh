...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v uname &>/dev/null || { echo "uname is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Show system information (jc uname parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --uname 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_system_main "$@"
