#!/usr/bin/env bash
function __bu_bu_get_file_stat_main()
{
...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v stat &>/dev/null || { echo "stat is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Show file metadata (jc stat parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --stat 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_file_stat_main "$@"
