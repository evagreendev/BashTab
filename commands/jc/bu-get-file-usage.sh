...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v du &>/dev/null || { echo "du is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Show disk usage by file or directory (jc du parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --du 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_file_usage_main "$@"
