#!/usr/bin/env bash
# Dispatch: source
# Synopsis: Show shell command history
function __bu_bu_get_history_main()
{
...
then
    bu_autohelp \
        --description "Show interactive shell command history (history builtin, PowerShell Get-History). Wraps history; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --history 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_history_main "$@"
