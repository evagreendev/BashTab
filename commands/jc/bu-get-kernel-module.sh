#!/usr/bin/env bash
# Dispatch: source
# Synopsis: List loaded kernel modules
function __bu_bu_get_kernel_module_main()
{
...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v lsmod &>/dev/null || { echo "lsmod is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "List loaded kernel modules (lsmod). Wraps lsmod; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --lsmod 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_kernel_module_main "$@"
