#!/usr/bin/env bash
# Dispatch: source
# Synopsis: List scheduled cron jobs
function __bu_bu_get_cron_job_main()
{
...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v crontab &>/dev/null || { echo "crontab is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "List cron jobs for the current user (PowerShell Get-ScheduledJob analog). Wraps crontab -l; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --crontab 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_cron_job_main "$@"
