...
then
    bu_autohelp \
        --description "Show operating system identification (/etc/os-release). Wraps cat /etc/os-release; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
local -a cmd=(cat /etc/os-release)
...
"${cmd[@]}" 2>/dev/null | jc --os-release 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_os_release_main "$@"
