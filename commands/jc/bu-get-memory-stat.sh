#!/usr/bin/env bash
function __bu_bu_get_memory_stat_main()
{
...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v vmstat &>/dev/null || { echo "vmstat is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Show virtual memory statistics (jc vmstat parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --vmstat 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_memory_stat_main "$@"
