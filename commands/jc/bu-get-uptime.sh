...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v uptime &>/dev/null || { echo "uptime is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "Show system uptime and load (jc uptime parser wrapper)." \
        --example "Default" "" \
...
"${cmd[@]}" 2>/dev/null | jc --uptime 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_uptime_main "$@"
