#!/usr/bin/env bash
# Synopsis: Show display and monitor information
function __bu_bu_get_display_main()
{
...
if [[ "$1" == "--is-compatible" ]]; then
    ...
    command -v xrandr &>/dev/null || { echo "xrandr is required" >&2; exit 1; }
    exit 0
fi
...
then
    bu_autohelp \
        --description "List displays and screen resolutions (xrandr). Wraps xrandr; extra arguments replace the default arguments." \
        --example "Default" ""
    return 0
fi
...
"${cmd[@]}" 2>/dev/null | jc --xrandr 2>/dev/null | jq -c 'if type == "array" then .[] else . end' 2>/dev/null | bu_out --format "$format"

bu_scope_pop_function
}

__bu_bu_get_display_main "$@"
