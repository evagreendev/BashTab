...
rl.on("line", (line) => {
    ...
    // Word-split the after-pipe portion
    const cmdWords = pipeAfter.trim().split(/\s+/).filter(w => w.length > 0);
    ...
    // For the command name: the first word of pipeAfter that's not an env-var assignment
    let envVars = "";
    let cmdName = "";
